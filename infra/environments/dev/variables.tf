# ==============================================================
# DEV variables. Defaults encode the dev environment so a plan
# works with only the three secrets supplied via terraform.tfvars.
# ==============================================================
...
variable "vpc_cidr" {
  description = "VPC CIDR — dev uses 10.0.0.0/16"
  type        = string
  default     = "10.0.0.0/16"
  nullable    = false
}
...
variable "capacity_type" {
  description = "Node capacity type — SPOT in dev for cost savings"
  type        = string
  default     = "SPOT"
  nullable    = false
}
...
variable "endpoint_public_access" {
  description = "Expose the EKS API endpoint publicly (true in dev for local kubectl)"
  type        = bool
  default     = true
  nullable    = false
}
...
variable "recovery_window_in_days" {
  description = "Secrets Manager recovery window — 0 in dev for immediate deletion"
  type        = number
  default     = 0
  nullable    = false
}
...
variable "gitops_target_revision" {
  description = "Git branch the root app tracks — dev tracks main"
  type        = string
  default     = "main"
  nullable    = false
}
