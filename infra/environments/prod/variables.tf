# ==============================================================
# PROD variables. Defaults encode the hardened prod environment:
# ON_DEMAND capacity, larger nodes, private API endpoint, and a
...
variable "vpc_cidr" {
  description = "VPC CIDR — prod uses 10.2.0.0/16"
  type        = string
  default     = "10.2.0.0/16"
  nullable    = false
}
...
variable "capacity_type" {
  description = "Node capacity type — ON_DEMAND in prod for stability"
  type        = string
  default     = "ON_DEMAND"
  nullable    = false
}
...
variable "endpoint_public_access" {
  description = "Expose the EKS API endpoint publicly — false in prod (VPN/VPC access only). Run Terraform from within the VPC or a peered network."
  type        = bool
  default     = false
  nullable    = false
}
...
variable "recovery_window_in_days" {
  description = "Secrets Manager recovery window — 30 days in prod"
  type        = number
  default     = 30
  nullable    = false
}
...
variable "gitops_target_revision" {
  description = "Git branch the root app tracks — prod tracks prod"
  type        = string
  default     = "prod"
  nullable    = false
}
